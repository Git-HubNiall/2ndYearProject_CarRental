...
public static final List<ItemOnSale> findAll(String filter) {
   return ItemOnSale.find.query().where().ilike("name", "%" + filter + "%").orderBy("name asc").findList();
}


public static List<ItemOnSale> findFilter(Long catID, String filter)
{
    return ItemOnSale.find.query().where().eq("categories.id", catID).ilike("name", "%" + filter + "%").orderBy("name asc").findList();
